...
* **Phase 16 Merge-Commit:** `56d3ee5` — `merge: Phase 16 — PWA app feel`
...
* **Gemergte Phasen:** 1 · 2 · 3 · 4 · 5 · 6 · 7A · 7B/7B1 · 7C · 8 · 9 · 10 · 11 · 12 · 13 · 14 · 15 · **16**
...
### Phase 16 — PWA / App-Gefühl

* **Web App Manifest** — `src/app/manifest.ts` (Next.js native, keine externen Pakete):
  * `name: "MUSKELKATER GMBH"`, `short_name: "Muskelkater"`
  * `display: "standalone"` — öffnet ohne Browser-Chrome
  * `start_url: "/heute"` — direkt ins Dashboard
  * `background_color: "#0a0a0b"`, `theme_color: "#0a0a0b"` (Dark Premium)
  * Manifest öffentlich via `/manifest.webmanifest` — in `src/proxy.ts` Matcher explizit von Auth-Guard ausgenommen
* **Icons** — `public/icons/icon-192.png` + `public/icons/icon-512.png`:
  * Generiert mit `scripts/generate-pwa-icons.ts` (reines Node.js, nur `zlib`, kein npm-Paket)
  * Design: dunkler Hintergrund `#141518` + zentriertes rotes Roundrect `#e0223a`
  * 512px: `purpose: "maskable"` (Android Adaptive Icons)
* **iOS Add-to-Homescreen Metadata** in `src/app/layout.tsx`:
  * `appleWebApp: { capable: true, statusBarStyle: "black", title: "Muskelkater" }`
  * `icons.apple` auf `icon-192.png`
  * `manifest: "/manifest.webmanifest"` Link-Tag
* **`viewport-fit=cover`** in `Viewport` Export — aktiviert `env(safe-area-inset-*)` für iOS-Geräte
* **Safe-Area Bottom Padding** — `@utility pb-safe-tabbar` in `globals.css`:
  * `padding-bottom: calc(7rem + env(safe-area-inset-bottom))`
  * `src/app/(app)/layout.tsx`: `pb-28` → `pb-safe-tabbar`, Desktop-Override `md:pb-12` bleibt
  * MobileTabbar hatte `h-[env(safe-area-inset-bottom)]` bereits — funktioniert jetzt korrekt mit `viewport-fit=cover`
* **Kein Service Worker**, kein Offline-Support, keine Push Notifications
* **Bugfix** `src/proxy.ts`: `manifest\\.webmanifest` in Matcher-Ausschluss aufgenommen (sonst 307 auf Auth für unauthentifizierte PWA-Clients)
* **Smoke-Script:** `scripts/qa-phase16-pwa-app-feel.ts` — 54 Tests PASS
* **Merge-Commit:** `56d3ee5`
* **Production live:** `https://muskelkater-gmbh.vercel.app` (`dpl_jQdDH2tGHtsBCT8Jz35sBpnbxf6Q`)
...
* **`/ernaehrung` — echte private Tages-Mahlzeitenlog-Seite** (ersetzt Basis aus Phase 5):
  * Vier Karten: Tagesziele (Kalorien/Protein/Wasser mit Fortschrittsbalken), Heute (Mahlzeiten-Liste + Formular), Wasser (WaterTracker), Letzte 7 Tage (NutritionWeekHistory)
  * Kalorien-Fortschrittsbalken erscheint nur wenn `currentCalories > 0` — sonst „Ziel: X kcal"
* **Neue DB-Tabelle `meal_log`** — individuelle Mahlzeiten-Einträge:
  * `id`, `user_id`, `log_date` (date), `meal_type` (text), `title` (text), `calories_kcal` (nullable integer), `protein_g` (nullable integer), `created_at`, `updated_at`
  * RLS owner-only Policy `meal_log_owner` (`auth.uid() = user_id`) — verifiziert via Supabase MCP
  * Zwei Indizes: `idx_meal_log_user` + `idx_meal_log_date` (user_id, log_date)
* **Migration** `drizzle/0007_phase14_meal_log.sql` — via Supabase MCP `apply_migration` in Production angewendet
* **Mahlzeit-Typen:** `fruehstueck` · `mittagessen` · `abendessen` · `snack` (4 Enums, kein DB-Enum — plain text mit Zod-Enum-Validierung)
* **Server Actions** in `src/app/(app)/ernaehrung/actions.ts`:
  * `addMealLog(rawInput)` — Zod-validiert; schreibt in `meal_log`; ruft `syncDailyNutritionTotals` auf
  * `deleteMealLog(rawInput)` — owner-only (`eq(mealLog.userId, userId)`); ruft `syncDailyNutritionTotals` auf
  * `syncDailyNutritionTotals(userId, logDate)` — berechnet `SUM(calories)` + `SUM(protein)` aus `meal_log`; upsert in `daily_nutrition_log`; setzt `mealsStatus.meal_log_active = true` wenn Einträge vorhanden, entfernt Key wenn leer
* **Sync-Logik → Team-Scoring:**
  * `meal_log_active: true` in `mealsStatus`-JSONB → `nutritionLogged = true` in `getMemberWeeklySignals`
  * Team sieht **ausschließlich** `nutritionLogged: boolean` — keine `title`, `mealType`, `caloriesKcal`, `proteinG`
  * Privacy `showNutrition = false` → `nutritionLogged = false` in maskiertem Status (unverändert)
* **Neue Komponenten:**
  * `src/components/nutrition/meal-log-form.tsx` (Client) — Radio-Buttons für Typ, Text-Input für Titel, Number-Inputs für Kalorien/Protein (optional), `useActionState`
  * `src/components/nutrition/meal-log-list.tsx` (Client) — Liste mit Typ-Icon (🌅☀️🌙🍎), Titel, optional Makros; Delete-Button mit `useTransition`
  * `src/components/nutrition/nutrition-week-history.tsx` (Server) — 7-Tage-Grid (Mo–So), heute hervorgehoben, CheckCircle2/Circle, Mahlzeiten-Anzahl
* **Modifizierte Dateien:**
  * `src/db/schema.ts` — `mealLog` Tabelle hinzugefügt
  * `src/lib/validation/nutrition.ts` — `addMealLogSchema`, `deleteMealLogSchema`, `MEAL_TYPE_LABELS`
  * `src/lib/nutrition/nutrition-data.ts` — `getTodayMealLogs()`, `getWeekNutritionSummary()`, `MealLogEntry`/`DayNutritionSummary` Interfaces
  * `src/components/nutrition/nutrition-goals.tsx` — Kalorien-Fortschrittsbalken nur wenn `currentCalories > 0`
  * `src/app/(app)/ernaehrung/page.tsx` — 4 Karten, 4 parallele Loads, kein MealChecklist/ProteinLog mehr
  * `eslint.config.mjs` — `muskelkater-gmbh/**` zu globalIgnores hinzugefügt (pre-existing lint-Fehler in untracked Dir)
* **Zod v4:** `addMealLogSchema` und `deleteMealLogSchema` nutzen `.issues[0]?.message`; UUID-Validation erfordert korrektes Variant-Byte (8/9/a/b)
* **Smoke-Script:** `scripts/qa-phase14-nutrition-v2.ts` — 41 Tests PASS
* **Merge-Commit:** `efa1b86`
* **Production live:** `https://muskelkater-gmbh.vercel.app`
...
| `56d3ee5` | merge: Phase 16 — PWA app feel |
| `5770707` | fix: exclude manifest.webmanifest from auth proxy matcher |
| `64a9532` | feat: add phase 16 pwa app feel |
...
* Ernährung V2: Mahlzeiten-Log (Typ, Titel, optional Kalorien/Protein), Tagesziele mit Fortschrittsbalken, 7-Tage-Übersicht
* Habits abhaken
...
* **PWA installierbar** — Web App Manifest (`/manifest.webmanifest`), App-Icons 192/512px, `display: standalone`, iOS Add-to-Homescreen, safe-area-aware Tabbar
...
npx tsx scripts/qa-phase16-pwa-app-feel.ts
```

---

## Testergebnisse Phase 16

| Check | Ergebnis |
|-------|----------|
| `npm run build` | PASS (21 Routen, `/manifest.webmanifest` Static) |
| `npx tsc --noEmit` | PASS |
| `npm run lint` | PASS |
| Phase 16 PWA Smoke | PASS (54 Tests) |
| Phase 15 Coach Smoke | PASS (661 Tests) |
| Vercel Production Deploy | PASS (`dpl_jQdDH2tGHtsBCT8Jz35sBpnbxf6Q`) |
| Live-Check `/login` | PASS (200) |
| Live-Check `/manifest.webmanifest` | PASS (200, name/short_name/display/icons korrekt) |
| Live-Check `/heute` (Redirect) | PASS (307 → `/login`) |
| Manueller Browser-Test | PASS (Android/iOS Add-to-Homescreen, standalone, safe-area Tabbar) |
...
| Check | Ergebnis |
|-------|----------|
| `npm run build` | PASS (20 Routen, `/ernaehrung` Dynamic) |
| `npx tsc --noEmit` | PASS |
| `npm run lint` | PASS (`muskelkater-gmbh/**` jetzt in globalIgnores) |
| Phase 14 Nutrition V2 Smoke | PASS (41 Tests) |
| Phase 13 Privacy/Profile Smoke | PASS (35 Tests) |
| Phase 12 Challenge Smoke | PASS (28 Tests) |
| Vercel Production Deploy | PASS (`dpl_FE8TzjsLsLhknjEotkVFemzuLhT9`) |
| Live-Check `/login` | PASS (200) |
| Live-Check `/heute` (Redirect) | PASS (307 → `/login`) |
| Live-Check `/team` (Redirect) | PASS (307 → `/login`) |
| Manueller Browser-Test | PASS (Mahlzeit hinzufügen/löschen, Typ-Auswahl, Kalorien/Protein optional, Fortschrittsbalken, 7-Tage-Übersicht, nutritionLogged auf `/heute`, Team kein Meal-Detail, Privacy showNutrition) |
...
**Phase 16 ist abgeschlossen und live.**

**Kandidaten für Phase 17:**
